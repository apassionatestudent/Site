...
const TESDAClassDetail = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        console.error("Fetch TESDA class detail error:", error);
  ...
  // => Postgres TIME columns come back as "HH:MM:SS" - convert to 12-hour display
  ...
  return (
    <div className="class-detail-page">
      ...
      <div className="class-detail-header">
        <h1>{batch.course_title || "Untitled Course"}</h1>
        <span className="class-detail-status">{batch.status}</span>
      </div>

      <p className="class-detail-batchname">{batch.batch_name} &middot; {batch.class_type}</p>
      ...
      <div className="class-detail-info-grid">
        ...
        <div className="class-detail-info-item">
          <img src={TrainerIcon} alt="Trainer" className="class-detail-info-icon" />
          <div>
            <span className="class-detail-info-label">Trainer</span>
            <span className="class-detail-info-value">{batch.trainer_name || "Not yet assigned"}</span>
          </div>
        </div>
            ...
            <tbody>
              {sessions.map((session) => (
                <tr key={session.session_id}>
                  ...
                  <td>{session.trainer_name || batch.trainer_name || "TBD"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default TESDAClassDetail;